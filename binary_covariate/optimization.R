...
for(k in 1:n1){
  ...
  val1[k] <- get.value(a1[k,],params_fit,bi_ind,m=m,utility=utility,indX_policy=indX_policy)[1]
  if(k%%10==0){
...
for(step in 1:n2){  
  ...
  a      <- rbind(a,cana)
  val2   <- get.value(cana,params_fit,bi_ind,m=m,utility=utility,indX_policy=indX_policy)[1]
  val    <- c(val,val2)
