...
#  
#  (X,Y[1]) ~ N(b0,s0)
#  log(delta[t]) ~ N(X1[,t]%*%b1,s1^2)
#  Y[t]     ~ N(X2[,t]%*%b2,s2^2)
#
#  X1[,t]   = (X,Y[t],log(A[t]),X*log(A[t]),Y[t-1]*log(A[t]))
#  X2[,t]   = (X,Y[t-1],delta[t],X*delta[t],Y[t-1]*delta[t]) 
#
#########################################################


MCMC_DPM <- function(base,nt,XX1,XY1,YY1,XX2,XY2,YY2,
                     L=5,DV=.1,eps=1,
  ...
  # Keep track of posterior samples
  ...
  for(iter in 1:iters){
    ...
    # Delta parameters
    # Update compliance model random effects
    SS <- sum(YY1)
    for(l in 1:L){
      id  <- which(g==l)
      ng  <- length(id)
    ...
    # Update compliance model variance
    Q1  <- rgamma(1,sum(nt)/2+eps,SS/2+eps)
    
    # Y parameters
    # Update disease progression model random effects
    SS <- sum(YY2)
    for(l in 1:L){
      id  <- which(g==l)
      ng  <- length(id)
    ...
    # Update disease progression model variance
