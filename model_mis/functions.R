...
gen.one.sub <- function(params,policy=riskscore,alpha=NULL){
  ...
  while(t<60 | length(Y)<3){
    ...
    Xy    <- c(1,X,lagY,(curd-6),X*(curd-6),(curd-6)*lagY,curd^2,lagY^2)
...
# using the true generate model
gen.m.subs.true <- function(m,params,alpha=NULL,policy=riskscore,utility="red",indX_policy){
  ...
  while(min(t)<60 | visit<3){
    ...
    #int2  <- sweep(X,1,lagY,"*")
    Xy    <- cbind(1,X,lagY,(curd-6),int1,(curd-6)*lagY,curd^2,lagY^2)
...
gen.m.subs <- function(m,params,alpha=NULL,policy=riskscore,utility="red",indX_policy){
...
# Compute the true value of a policy defined by a using true generative model
get.value.true <- function(a,params,m=5000,policy=riskscore,thresh=NULL,utility="red",indX_policy){
  
  # Estimate the threshold that gives average recommended recall time = 6
  if(is.null(thresh)){
    thresh <- get.thresh.true(a,params,policy=policy,indX_policy=indX_policy)
  }
  alpha   <- c(thresh,a)
  dat     <- gen.m.subs.true(m,params,alpha=alpha,policy=policy,utility=utility,indX_policy=indX_policy)  
...
# Compute the value of a policy defined by a using the proposed model
get.value <- function(a,params,m=5000,policy=riskscore,thresh=NULL,utility="red",indX_policy){
  
  # Estimate the threshold that gives average recommended recall time = 6
  if(is.null(thresh)){
    thresh <- get.thresh(a,params,policy=policy,indX_policy=indX_policy)
  }
  alpha   <- c(thresh,a)
  dat     <- gen.m.subs(m,params,alpha=alpha,policy=policy,utility=utility,indX_policy=indX_policy)  
  results <- c(mean(dat$utility),se(dat$utility),
               mean(dat$cost),se(dat$cost))
  names(results)<-c("Value","SE(Value)","Cost","SE(Cost)")
...
# Estimate the true threshold satisfying the average recall time constraint 
# for a policy defined by a using the true generative model
get.thresh <- function(a,params,m=2000,policy=riskscore,npts=8,indX_policy){
  ...
  for(ttt in 1:ntau){
    dat      <- gen.m.subs.true(m=m,params=params,
...
# Get the threshold satisfying the average recall time constraint 
# for a policy defined by a using the proposed model
get.thresh <- function(a,params,m=2000,policy=riskscore,npts=8,indX_policy){
  ...
  for(ttt in 1:ntau){
    dat      <- gen.m.subs(m=m,params=params,
                           alpha=c(cant[ttt],a),policy=policy,indX_policy=indX_policy)  
    ccc[ttt] <- mean(dat$cost)
  }
  if(ntau>6){ccc <- loess(ccc~cant)$fitted}
